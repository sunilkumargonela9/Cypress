...
import * as data from '../../fixtures/suncorp/config_overdraft.json'
...
describe('Verify that the Pricing Page Wizard Page should have the Fees and Cost table configuratrion', () => {
    ...
    it('Verify Fees Table Configuration configuration for Line Fee', () => {
        ProductPage.validateFeeName("Facility Fee (Overdraft > $500K)", data)
        ProductPage.validatePaymentFreq("Facility Fee (Overdraft > $500K)", data, "enabled")
        ProductPage.validateTableDdlFieldOptions("Facility Fee (Overdraft > $500K)", data)
        ProductPage.validateStdFee("Facility Fee (Overdraft > $500K)", data, "disabled")
        ProductPage.validateProposedFee("Facility Fee (Overdraft > $500K)", data, "enabled")
        ProductPage.validateFeeIfWaived("Facility Fee (Overdraft > $500K)", data, "enabled")
        ProductPage.validateFeeIfAmortised("Facility Fee (Overdraft > $500K)", data, "disabled")
    });
    ...
    it('Verify Fees Table Configuration configuration for Unused Limit Fee', () => {
        ProductPage.validateFeeName("Business Ready Package Fee", data)
        ProductPage.validatePaymentFreq("Business Ready Package Fee", data, "enabled")
        ProductPage.validateTableDdlFieldOptions("Business Ready Package Fee", data)
        ProductPage.validateStdFee("Business Ready Package Fee", data, "disabled")
        ProductPage.validateProposedFee("Business Ready Package Fee", data, "enabled")
        ProductPage.validateFeeIfWaived("Business Ready Package Fee", data, "enabled")
        ProductPage.validateFeeIfAmortised("Business Ready Package Fee", data, "disabled")
    });

    it("verify Brokerage dollar amount field configuration and rule is correct", () => {
        ProductPage.enterMandatoryFields('suncorp_qa', 'Overdraft')
        PricingPage.selectProduct('Overdraft')
